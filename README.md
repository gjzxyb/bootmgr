...
- 镜像文件 SHA256/大小校验，以及部署前镜像、服务器、部署网段、模板和已配置 BMC Endpoint 连通性检查
...
- 未配置 BMC Endpoint 的实体机不会被部署预检阻断，会进入人工开机/PXE、metadata 访问、SSH 或物理 full evidence 的闭环流程；API 重启后会自动恢复 pending 的无 BMC 物理部署 workflow
...
系统管理页“真实验收”会调用 `/api/v1/system/lab-validation` 汇总 PXE/BMC/SSH 验收状态，并通过目标矩阵按资产展示 PXE、BMC、SSH、证据状态、最近验收批次/结果、全链路 ready 标记和缺口原因；BMC 是可选能力：有 BMC Endpoint 的资产会严格要求 Redfish/IPMI proof，没有 BMC Endpoint 的资产可通过 PXE+真实 SSH+full evidence 完成可用闭环。有 `primary_mac` 的资产即使尚未产生 BootEvent，也会进入候选目标，PXE BootEvent 可通过 `server_id` 或事件 MAC 匹配库存 `primary_mac` 计入该目标，并可从行内直接发起严格检查或补录证据。BootEvent 会记录来源：`http_ipxe` 表示来自已启用 `deployment` 网络 CIDR 的客户端请求 iPXE 脚本，`pxe_dhcp` 表示内置 DHCP/ProxyDHCP 收到真实 PXE 请求，`api_event` 表示手工 API 补录、部署网段外的 HTTP iPXE 请求或带 `X-Lab-Validation-Probe: 1` 的内部验收 HTTP 探测；严格 PXE、`ok` 级 PXE/full 证据和全链路 ready 只接受 7 天内 `http_ipxe`/`pxe_dhcp` 这类真实启动链路来源，目标矩阵会优先采用最新真实 PXE 事件，后续 `api_event` 不会覆盖已有真实启动证据；`api_event`、空来源和 `unknown` 只用于兼容历史、补录和排障。执行检查需要管理员角色和 `X-Confirm-Action: system.lab-validation.run`，会只读探测 `BOOT_BASE_URL/boot/ipxe`、已启用的 DHCP/ProxyDHCP bootfile 响应、已启用的 TFTP `boot.ipxe`、已配置的 BMC/SSH 连通性，不执行电源动作，也不动态启用 DHCP/TFTP。严格 BMC 检查会继续执行只读固件/身份探针，Redfish 读取 Systems/Managers，IPMI 读取 `mc info`，批次结果会记录厂商、型号、序列号和版本摘要，并把这些非敏感身份字段写入 run result `details`；未配置 BMC Endpoint 的请求资产会把 BMC 项记为 skipped。SSH 检查会执行单行只读探针命令，默认 `printf 'ok '; hostname; id -un; uname -srm`，也可通过 `ssh_probe_command` 覆盖，批次结果会记录 stdout/stderr/exit_code 摘要，并把 command、exit_code、stdout、stderr 和错误摘要写入 run result `details`。合成 DHCP 探测不会写入 BootEvent，HTTP 探测也会强制降级为 `api_event`；执行检查时可开启 `strict=true`，要求 `pxe_macs` 指向真实 PXE 客户端、`server_ids` 指向真实资产，且只有请求范围内存在 BMC Endpoint 时才要求 `BMC_ADAPTER=redfish|ipmi`。当严格运行同时检查 PXE 和 SSH 且传入 `server_ids` 时，会为每台资产额外写入 `full_chain_target` 结果；有 BMC Endpoint 的目标要求 PXE+BMC+SSH 和新鲜完整物理证据闭环，无 BMC Endpoint 的目标要求 PXE+SSH 和新鲜完整物理证据闭环。执行检查时可填写 `server_ids` 把 BMC/SSH 限定到指定真实资产，也可填写 `pxe_macs` 校验指定真实 PXE 客户端是否已产生启动事件。每次执行都会生成 `run_id` 并持久化批次、目标、请求人、Request ID 和单项结果，系统管理页会展示最近 10 次运行，并可通过 `/api/v1/system/lab-validation/runs/{id}/evidence-bundle` 查看该批次的环境快照、检查结果、关联 BootEvent、BMC、SSH、真实终端会话 transcript、脚本执行结果、日志事件和配置问题。真实 PXE 客户端、物理 Redfish/IPMI、真实 SSH 主机或全链路实验结果可通过 `POST /api/v1/system/lab-validation/evidence` 记录非敏感证据摘要，确认头为 `X-Confirm-Action: system.lab-validation.evidence`；证据可传 `run_id` 关联一次已完成且 7 天内的验收批次，`ok` 全链路证据必须传入一次严格 PXE+SSH 运行的 `run_id`，若目标有 BMC Endpoint 则该运行还必须包含 BMC 成功结果；`tools/physical-validation.ps1 -RecordEvidence` 可在严格运行后按现场清单自动补录已通过的 `pxe`、`bmc`、`ssh` 单项证据，`-RecordFullEvidence` 则补录全链路证据并复跑；`ok` 级证据必须绑定真实平台引用：PXE 证据传真实链路 `boot_event_id`，其中 `http_ipxe` 必须由部署网段内客户端 IP 自动分类产生；BMC/SSH 证据传 `server_id` 且对应最近连通检查为 `ok`，全链路证据同时传 `server_id` 与真实链路 `boot_event_id`。`physical_evidence` 只把 7 天内且引用完整的 `ok` 证据计入通过。
...
验收包会额外返回 `operator_checklist` 和 `evidence_candidates`，按目标列出 PXE BootEvent、BMC 身份探针、SSH 命令证明和 full-chain 证据引用的完成状态、下一步动作、阻塞原因和后端生成的可记录证据草案；BMC 清单项只有在目标配置了 BMC Endpoint 且本次运行结果成功、`details` 含结构化身份字段时才标记为 `ok`，没有 BMC Endpoint 的目标会标记为 `skipped` 且不阻断 full-chain 候选；SSH 清单项只有在 `details` 含 `host_key_policy=known_hosts`、`host_key_verified=true`、`host_key_sha256`、command、exit_code 和 stdout 时才标记为 `ok`，便于物理实验室现场复核。系统管理页打开验收包后，可从状态已满足的清单行或 `evidence_candidates` 直接预填并记录 PXE/BMC/SSH 单项证据；当同一资产的 PXE、SSH 都为 `ok`，且 BMC 为 `ok` 或 `skipped` 并有 BootEvent 引用时，也可从候选项预填全链路证据，最终仍由后端 evidence API 执行真实引用校验。
...
10. 在“资产管理”为带管理口的目标服务器配置 BMC Endpoint；没有 BMC 的实体机可保持未配置，后续通过 PXE、人工开机和 SSH/证据闭环完成部署验收。
11. 在“部署任务”选择一台或多台服务器、镜像、部署网络、擦除策略并确认重装风险后创建部署；无 BMC 目标预检会显示人工开机/PXE 现场动作。
12. 等待工作流推进，打开部署详情查看步骤、耗时、运行尝试历史；无 BMC 目标会显示当前等待的人工 PXE、metadata 或 SSH/物理证据动作。
...
- BMC、部署工作流和监控指标默认使用模拟实现，用于验证平台业务闭环；配置 `BMC_ADAPTER=redfish` 或 `ipmi` 后会连接物理 BMC，配置 `COLLECTOR_MODE=ssh` 后会通过 Go SSH 采集真实主机指标。部署创建要求平台已配置启用的部署网段、镜像已校验、模板启用并显式确认擦除/重装风险；目标服务器如果已配置 BMC Endpoint，会调用当前 `BMC_ADAPTER` 做连通性检查，未配置 BMC Endpoint 的实体机不会因此被拦截，可通过 PXE 启动、人工电源操作和 SSH/物理证据完成闭环，创建前预检会返回 `power_control=manual` 和现场 `operator_actions`，创建后部署详情会显示当前等待的人工 PXE、metadata 或 SSH/物理证据动作。请求可传 `network_id` 绑定本次部署使用的启用 `deployment` 网络，未传时保留旧的自动选择兼容行为，且请求必须包含 `erase_confirmed=true` 和擦除策略记录。系统管理页可对单条网络配置执行只读检查，覆盖 CIDR、重叠、网关、DNS、状态、DHCP/ProxyDHCP 和部署网可用性；真实 DHCP/TFTP 监听只由 `BOOT_SERVICES_ENABLED=true` 启动。批量部署接口单批最多 20 台，全部目标 preflight 通过后才会创建部署，并可使用同一个 `network_id`。部署工作流受 `DEPLOYMENT_CONCURRENCY` 控制，超出并发的任务保持 `pending` 排队，拿到执行槽后才进入 `running`。工作流模板 action 可使用 `simulate_failure` 演示失败状态，失败或已取消部署可通过 `X-Confirm-Action: deployment.retry` 按原擦除策略重新执行；部署日志接口会保留所有 workflow run 尝试历史，并返回最新 run 的任务统计和 `duration_ms`。
